...
## 💡 핵심 코드 분석
### 1. `CustomMapView.swift` - 데이터 동기화 델리게이트
지도 중심 좌표가 변경될 때마다 뷰모델의 상태를 갱신하여 데이터 무결성을 보장하는 핵심 로직입니다.
```swift
func mapView(_ mapView: MKMapView, regionDidChangeAnimated animated: Bool) {
    // 지도를 움직일 때마다 뷰모델의 centerCoordinate를 실시간 업데이트
...
### 2. `RecordListView.swift` - 실시간 필터링 엔진
연산 프로퍼티를 통해 카테고리와 검색어를 결합하여, 리스트 데이터를 실시간으로 필터링하는 파이프라인입니다.
```swift
var filteredRecords: [VisitRecord] {
    ...
    if !query.isEmpty {
        records = records.filter { record in
            record.title?.localizedCaseInsensitiveContains(query) ?? false ||
...
### 3. `RecordEditView.swift` - 데이터 수정 및 정합성 보장
데이터 수정 시 기존 레코드를 삭제 후 새로운 데이터를 저장하여 로컬 데이터베이스의 일관성을 유지하는 로직입니다.
```swift
private func saveAction() {
    // 기존 레코드 삭제 후 신규 데이터 저장 (데이터 정합성 유지)
    ...
    viewModel.addRecord(
        ...
        memo: memoText.isEmpty ? "메모 없음" : memoText,
...
## 📈 브랜치 전략 (Git Flow)
본 프로젝트는 효율적인 개발과 코드 안정성을 위해 다음과 같은 브랜치 전략을 사용했습니다.
